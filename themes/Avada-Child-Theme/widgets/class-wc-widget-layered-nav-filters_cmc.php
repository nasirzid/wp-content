 ...
 */
class WC_Widget_Layered_Nav_Filters_CMC extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'CMC: Display a list of active product filters.', 'woocommerce' );
		$this->widget_id          = 'woocommerce_layered_nav_filters_cmc';
		$this->widget_name        = __( 'CMC: Active Product Filters', 'woocommerce' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
	...
	public function widget( $args, $instance ) {
		...
		$s_keyword          = isset( $_GET['filter_s'] ) ? wc_clean( wp_unslash( $_GET['filter_s'] ) ) : ''; // WPCS: input var ok, CSRF ok.
		...
		if ( 0 < count( $_chosen_attributes ) || 0 < $min_price || 0 < $max_price || ! empty( $rating_filter ) ) {
			...
			if ( ! empty( $_chosen_attributes ) ) {
				foreach ( $_chosen_attributes as $taxonomy => $data ) {
					foreach ( $data['terms'] as $term_slug ) {
						...
						/*$link  = str_replace('?s=', '?filter_s=', $link);
						$link  = str_replace('&s=', '&filter_s=', $link);*/
			...
			if ( ! empty( $s_keyword ) ) {
				/* translators: %s: rating */
				$link = remove_query_arg( 'filter_s', $base_link );
				$link = remove_query_arg( 's', $link );
				/*$link  = str_replace('?s='.$s_keyword, '?', $link);
				$link  = str_replace('&s='.$s_keyword, '', $link);*/
				echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . sprintf( esc_html__( '%s', 'woocommerce' ), esc_html( $s_keyword ) ) . '</a></li>';
