			...
			exitBuilder: function( event ) {
				...
				if ( ! linkTag.parent().hasClass( 'exit-to-dashboard' ) ) {
					if ( linkTag.parent().hasClass( 'exit-to-back-end' ) ) {
						link += '?post=' + FusionApp.getPost( 'post_id' ) + '&action=edit';
					} else {
			...
			getLanguageLabel: function( data, id ) {
				...
				if ( this.languageFlags ) {
					if ( 'undefined' !== typeof data.flag ) {
						$languageLabel = '<img src="' + data.flag + '" /> ';
					}
					if ( 'undefined' !== typeof data.country_flag_url ) {
						$languageLabel = '<img src="' + data.country_flag_url + '" /> ';
					}
				}
	...
	FusionPageBuilder.Dialog = Backbone.Model.extend( {
		...
		dialogTabs: function( thisEl ) {
			thisEl.find( '.fusion-tabs-menu a' ).on( 'click', function( event ) {
				...
				thisEl.closest( '.fusion-sidebar-section, .ui-dialog-content' ).scrollTop( 0 );
			} );
			...
			jQuery.each( dependencies, function( index, dependency ) {
				...
				if ( 'undefined' === typeof parentValue ) {
					if ( 'TO' === self.type ) {
						...
						parentValue = 'undefined' === typeof FusionApp.data.postMeta[ setting ] ? FusionApp.data.postMeta[ 'pyre_' + setting ] : FusionApp.data.postMeta[ setting ];
					}
				}
...
FusionPageBuilder.fusionActiveStates = {
	...
	triggerActiveState: function( data ) {
		var self = this,
			...
			$target   = 'string' === typeof data.selector && -1 !== data.selector.indexOf( '$el' ) ? $targetEl : $targetEl.find( '.fusion-builder-element-content ' + data.selector ),
			animationDuration;

		$target   = ! $target.length ? $targetEl.find( '.fusion-builder-child-element-content ' + data.selector ) : $target;
...
;/* global builderConfig, FusionPageBuilder, builderId, fusionSettings, FusionPageBuilderApp, fusionAllElements, fusionAppConfig, FusionApp, fusionOptionName, fusionBuilderText */
/* jshint -W020 */
		...
		var fusionApp = Backbone.Model.extend( { // jshint ignore: line
			...
			savePostContent: function( successAction ) {
				...
				if ( 'object' === typeof successAction && 'undefined' !== typeof successAction.action && ( 'switch_page' === successAction.action || 'exit_builder' === successAction.action ) ) {
					...
					jQuery( '.fusion-builder-confirmation-modal-save' ).attr( 'style', 'width:calc(100% - ' + jQuery( '.fusion-builder-save-page' ).outerWidth() + 'px);' );
				}
			...
			checkLegacyIcons: function( icon ) {
				var oldIconName;
		...
		createTypography: function( event ) { // jshint ignore: line
			var fusionTypographyForm = MediumEditor.extensions.form.extend( {
				...
				handleFontChange: function( event ) {
					...
					if ( -1 !== FusionApp.assets.webfontsStandardArray.indexOf( font ) ) {
				...
				updateVariants: function( font ) {
					...
					if ( ! hasRegular && holder.firstChild ) {
						holder.firstChild.classList.add( 'active' );
						defaultVal = holder.firstChild.getAttribute( 'data-value' );
						inputDiv.setAttribute( 'data-value', defaultVal );
						inputDiv.innerHTML = holder.firstChild.innerHTML;
		...
		createInlineShortcode: function( event ) { // jshint ignore: line
			var FusionInlineShortcodeForm = MediumEditor.extensions.form.extend( {
				...
				init: function() {
					...
					// Class applier for highligt element.
		...
		createAnchor: function( event ) { // jshint ignore: line
			var FusionAnchorForm = MediumEditor.extensions.form.extend( {
				...
				createForm: function() {
					...
					this.on( input, 'change', this._handleInputChange.bind( this ), true );
					this.on( input, 'blur', this._handleInputChange.bind( this ), true );
...
;/* global includesURL, fusionAllElements, FusionEvents, FusionPageBuilderViewManager, fusionBuilderText */
var FusionPageBuilder = FusionPageBuilder || {};
FusionPageBuilder.options = FusionPageBuilder.options || {};
...
FusionPageBuilder.options.fusionOptionUpload = {
			...
			$uploadButton.click( function( event ) {
				...
				fileFrame.on( 'open', function() {
					var selection = fileFrame.state().get( 'selection' ),
						...
						id        = $thisEl.closest( '.fusion-builder-module-settings' ).find( '#image_id' ).val(),
						attachment,
						parsedObject;
				...
				fileFrame.on( 'select insert', function() {
					...
					// Set image id.
					imageIDField = $thisEl.closest( '.fusion-builder-option' ).find( '#' + optionName + '_id' );

					if ( 'element_content' === optionName ) {
						imageIDField = $thisEl.closest( '.fusion-builder-option' ).find( '#image_id' );
					}
					...
					if ( 'undefined' === typeof state.get( 'selection' ) ) {
						...
						if ( multiUpload ) {
							...
							if ( firstElementNode.length ) {
								...
								if ( firstElement && 'undefined' === typeof firstElement.model.attributes.params[ $thisEl.data( 'param' ) ] ) {
	...
	fusionBuilderImagePreview: function( $uploadButton ) {
		...
		var $uploadField = $uploadButton.closest( '.fusion-upload-area' ).find( '.fusion-builder-upload-field' ),
			...
			imageURL     = $uploadField.hasClass( 'fusion-image-as-object' ) && value ? value.url : value,
		...
		if ( 'file' === $uploadButton.closest( '.fusion-upload-area' ).data( 'mode' ) ) {
			return;
		}
...
FusionPageBuilder.options.radioButtonSet = {
	optionRadioButtonSet: function( $element ) {
		var $radiobuttonsets, $radiobuttonset, $radiosetcontainer, optionId,
			self = this;
...
FusionPageBuilder.options.fusionRangeField = {
	optionRange: function( $element ) {
		...
		if ( $rangeSlider.length ) {
			...
			$rangeSlider.each( function() {
				...
				self.createSlider( $i, $targetId, $rangeInput, $min, $max, $step, $value, $decimals, $rangeCheck, $rangeDefault, $hiddenValue, $defaultValue, $direction );
	...
	createSlider: function( $slide, $targetId, $rangeInput, $min, $max, $step, $value, $decimals, $rangeCheck, $rangeDefault, $hiddenValue, $defaultValue, $direction ) {

		// Create slider with values passed on in data attributes.
		var self    = this,
			$slider = noUiSlider.create( self.$rangeSlider[ $slide ], {
				start: [ $value ],
				step: $step,
				direction: $direction,
				range: {
		...
		$rangeInput.closest( '.fusion-builder-option' ).attr( 'data-index', $slide );
		...
		if ( $rangeDefault ) {
			$rangeDefault.on( 'click', function( e ) {
				e.preventDefault();
				self.$rangeSlider[ $slide ].noUiSlider.set( $defaultValue );
		...
		$rangeInput.on( 'blur', function() {
			...
			if ( this.value !== self.$rangeSlider[ $slide ].noUiSlider.get() ) {

				// This triggers 'update' event.
				self.$rangeSlider[ $slide ].noUiSlider.set( this.value );
			}
		} );
	}
};
...
FusionPageBuilder.options.fusionRepeaterField = {
	...
	initRepeater: function( $repeater ) {
		...
		if ( 'object' === typeof values && rows ) {
			...
			rowTitle = 'undefined' !== typeof values[ option.bind_title ] && values[ option.bind_title ] ? values[ option.bind_title ] : '';
		...
		$repeater.on( 'change', '.repeater-row [name=' + option.bind_title + ']', function() {
			var title = jQuery( this ).hasClass( 'fusion-select-option' ) ? jQuery( this ).siblings( '.fusion-select-label[for=' + jQuery( this ).attr( 'id' ) + ']' ).html() : jQuery( this ).val();
	...
	createRepeaterRow: function( fields, values, $target, rowTitle ) {
		...
		if ( 'TO' !== this.context && 'FBE' !== this.context && 'undefined' !== typeof this.model && 'undefined' !== typeof this.model.get ) {
...
FusionPageBuilder.options.fusionSelectField = {
	optionSelect: function( $element ) {
		var $selectField;
		$element   = $element || this.$el;
		$selectField = $element.find( '.fusion-select-field:not(.fusion-select-inited):not(.fusion-form-multiple-select):not(.fusion-skip-init)' );
		...
		if ( $selectField.length ) {
			...
			$selectField.each( function() {
				...
				// Option is selected.
				$self.find( '.fusion-select-label' ).on( 'click', function() {
...
FusionPageBuilder.options.fusionTypographyField = {
	...
	renderFontSelector: function( $element ) {
		...
		_.each( jQuery( selectors ), function( selector ) {
			var fontFamily = self.getTypographyVal( selector, 'font-family' ).replace( /'/g, '"' ),
				id         = jQuery( selector ).closest( '.fusion-builder-option.typography' ).attr( 'data-option-id' );
	...
	renderBackupFontSelector: function( id, fontFamily ) {
		var self          = this,
			$option       = jQuery( '.fusion-builder-option.typography[data-option-id="' + id + '"] .font-backup' ),
			standardFonts = [],
			$fusionSelect;
		...
		$fusionSelect.find( '.fusion-select-option-value' ).on( 'change', function() {
			self.setTypographyVal( id, 'font-backup', jQuery( this ).val() );
		} );
	},
	...
	renderVariantSelector: function( id, fontFamily ) {
		...
		var self       = this,
			selector   = jQuery( '.fusion-builder-option.typography[data-option-id="' + id + '"] .variant select' ),
	...
	renderSubsetSelector: function( id, fontFamily ) {
		...
		var self       = this,
			...
			selector   = jQuery( '.fusion-builder-option.typography[data-option-id="' + id + '"] .subsets select' ),
			data       = [],
	...
	checkFontChanged: function( family, variant, subset, element ) {
		var id     = jQuery( element ).closest( '.fusion-builder-option.typography' ).attr( 'data-option-id' ),
...
var fusionSanitize = { // eslint-disable-line no-unused-vars
		...
		_.each( args.conditions, function( arg, i ) {
			if ( 'undefined' !== typeof arg[ 0 ] ) {
				...
				case 'in':
					...
					break;
				case 'true':
					checks[ i ] = ( self.getSettings()[ arg[ 0 ] ] === true || self.getSettings()[ arg[ 0 ] ] === 'true' || self.getSettings()[ arg[ 0 ] ] === 1 || self.getSettings()[ arg[ 0 ] ] === '1' || self.getSettings()[ arg[ 0 ] ] === 'yes' );
					break;
				}
			}
		} );
	 ...
	 * @param {string} args.element - The element we want to affect.
	 * @param {string} args.className - The class-name we want to toggle.
	 * @return {void}
	 */
