	...
	FusionPageBuilder.SidebarView = Backbone.View.extend( {
					...
					_.each( selectors, function( selector ) {
						...
						_.each( option.edit_shortcut.shortcuts, function( shortCut ) {
							...
							if ( hasAriaLabel ) {
								...
								if ( 'undefined' !== typeof shortCut.link ) {
									target   = 'undefined' !== typeof shortCut.target ? shortCut.target : '_blank';
									icon     = 'undefined' !== typeof shortCut.icon ? shortCut.icon : 'fusiona-pen';
									cssClass = 'undefined' !== typeof shortCut.css_class ? shortCut.css_class : '';

									html = '<a class="fusion-panel-shortcut ' + cssClass + '" href="' + shortCut.link + '" target="' + target + '" aria-label="' + shortCut.aria_label + '"><span class="' + icon + '"></span></a>';
								} else {
									icon   = 'undefined' !== typeof shortCut.icon ? shortCut.icon : 'fusiona-cog';
									if ( 'undefined' !== typeof shortCut.open_parent ) {
										openParent = ' data-fusion-option-open-parent="true"';
									}
									if ( 'undefined' !== typeof shortCut.callback ) {
										callback = ' data-callback="' + shortCut.callback + '"';
									}
									html = '<span class="fusion-panel-shortcut" data-fusion-option="' + option.id + '" aria-label="' + shortCut.aria_label + '"' + openParent + callback + '><span class="' + icon + '"></span></span>';
								}
								...
								if ( 'undefined' !== typeof shortCut.order ) {
									shortcuts[ selector ][ shortCut.order ] = html;
								} else {
		...
		shortcutClick: function( $trigger ) {
			if ( $trigger.attr( 'data-callback' ) && 'function' === typeof FusionApp.callback[ $trigger.attr( 'data-callback' ) ] ) {
				...
				this.openOption( $trigger.data( 'fusion-option' ), '', $trigger.data( 'fusion-option-open-parent' ) );
			}
		},
		 ...
		 *
		 * @since 2.0.0
		 * @param {string} option - The option we want to focus on.
		...
		scrollToElement: function( $element, smooth ) {
			var $section       = this.$el.find( '.fusion-sidebar-section:visible' ),
		...
		resizableDrag: function() {
			...
			$sidebar.resizable( {
				...
				resize: function( event, ui ) {
					var width = ( 320 >= ui.size.width ) ? 320 : ui.size.width;
					...
					try {
						...
						if ( 'object' === typeof data ) {
							this.panelData = {
								...
								width: 'undefined' !== typeof data.width ? data.width : 320,
	...
	window.avadaPanelIFrame = {
		...
		partialRefresh: function( id, partials, value, cid ) { // jshint ignore:line
			...
			if ( 'undefined' !== typeof partials ) {
				...
				itemID = itemID.replace( 'pyre_', '' );
				jQuery( 'li[data-option-id="' + itemID + '"]' ).addClass( 'partial-refresh-active' );
			}
			...
			jQuery.ajax( {
				...
				success: function( output ) {
					_.each( output, function( content, scopedID ) {
						...
						jQuery( 'li[data-option-id="' + itemID + '"]' ).removeClass( 'partial-refresh-active' );
		...
		/**
		 * Populates the mapToPo & mapPoTo maps.
		 *
		 * @return {void}
		 */
		populateToPoMaps: function() {
			var self = this;

			// Populate the maps to make things easier.
			if ( ! self.mapPoTo || ! self.mapToPo ) {
				self.mapPoTo = {};
				self.mapToPo = {};

				// Add items from settingsToPo to the flat map.
				_.each( FusionApp.settingsToPo, function( item ) {
					_.each( item, function( arrayItem ) {
						var po = arrayItem.option,
							to = arrayItem.to;
				...
				// Go through the rest of the post-meta and add them to the flat map.
				_.each( FusionApp.data.postMeta, function( val, metaID ) {
					metaID = metaID.replace( 'pyre_', '' );
					if ( 'undefined' === typeof self.mapPoTo[ metaID ] ) {
						self.mapPoTo[ metaID ] = metaID;
						self.mapToPo[ metaID ] = metaID;
					}
				} );
			}
		},
		 ...
		 * @param {string} type - PO/TO etc.
		 * @return {mixed} - Returns the value.
		 */
		getPoToValue: function( id, choice, type ) {
			var mapKey = this.getGlobalMapKey( type, id, choice );

			if ( 'TO' !== type && choice && fusionOptionNetworkNames[ mapKey ] && fusionOptionNetworkNames[ mapKey ].theme ) {
				if ( fusionOptionNetworkNames[ mapKey ].theme[ 1 ] && fusionOptionNetworkNames[ mapKey ].theme[ 1 ] !== choice ) {
					return null;
				}
			}
			if ( mapKey ) {
		 ...
		 */
		getGlobalMapKey: function( type, id, choice ) {
			var mapKey = id;

			// Make sure the maps are populated to make things easier.
			this.populateToPoMaps();

			// Small caching to avoid searching every time.
			this.savedMapReferences = this.savedMapReferences || {};
			this.savedMapReferences[ type ] = this.savedMapReferences[ type ] || {};
			if ( 'TO' === type && choice ) {
				if ( this.savedMapReferences[ type ][ id + '--' + choice ] ) {
					return this.savedMapReferences[ type ][ id + '--' + choice ];
				}
			} else if ( this.savedMapReferences[ type ][ id ] ) {
				return this.savedMapReferences[ type ][ id ];
			}

			// Get the global-map key if this is a PO.
			if ( 'PO' === type ) {
				mapKey = id.replace( 'pyre_', '' );
				_.find( fusionOptionNetworkNames, function( item, key ) {
					if ( item.post && ( id === item.post || id.replace( 'pyre_', '' ) === item.post ) ) {
						mapKey = key;
						return true;
					}
				} );
			...
			// Get the global-map key if this is a term option.
			if ( 'TAXO' === type ) {
				mapKey = id.replace( 'pyre_', '' );
				_.find( fusionOptionNetworkNames, function( item, key ) {
					if ( item.term && id === item.term ) {
						mapKey = key;
						return true;
					}
					if ( item.term && id.replace( 'pyre_', '' ) === item.term ) {
						mapKey = key;
						return true;
					}
				} );
			...
			// Get the global-map key if this is a TO.
			if ( 'TO' === type ) {
				_.find( fusionOptionNetworkNames, function( item, key ) {
					if ( 'object' === typeof item.theme && 'undefined' !== typeof item.theme[ 0 ] && item.theme[ 0 ] === id && 'undefined' !== typeof item.theme[ 1 ] && 'undefined' !== typeof choice && choice === item.theme[ 1 ] ) {
						mapKey = key;
						return true;
					}
				} );
				...
				_.each( [ 'is_home', 'is_tag', 'is_category', 'is_author', 'is_date', 'is_singular_post' ], function( condition ) {
					if ( FusionApp.data[ condition ] ) {
						_.find( fusionOptionNetworkNames, function( item, key ) {
							if ( item[ condition ] && ( id === item[ condition ] || id.replace( 'pyre_', '' ) === item[ condition ] ) ) {
								mapKey = key;
								return true;
							}
						} );
				...
				if ( -1 !== FusionApp.data.postDetails.post_id.toString().indexOf( '-archive' ) ) {
					_.find( fusionOptionNetworkNames, function( item, key ) {
						if ( 'undefined' !== typeof item.archive && ( 'undefined' !== typeof item.theme && id === item.theme ) ) {
							mapKey = key;
							return true;
						}
					} );
			...
			// Save in our cache for later use.
			if ( 'TO' === type && choice ) {
				this.savedMapReferences[ type ][ id + '--' + choice ] = mapKey;
			} else {
				this.savedMapReferences[ type ][ id ] = mapKey;
			}
			return mapKey;
		},
		...
		getPoToValueFromGlobalKey: function( key, choice ) {
			...
			if ( fusionOptionNetworkNames[ key ] ) {
				...
				if ( fusionOptionNetworkNames[ key ].theme ) {

					// If we have a TO value for defined theme-option name, get its value.
					if ( 'object' === typeof fusionOptionNetworkNames[ key ].theme && fusionOptionNetworkNames[ key ].theme[ 0 ] ) {
						if ( FusionApp.settings[ fusionOptionNetworkNames[ key ].theme[ 0 ] ] ) {
							value = FusionApp.settings[ fusionOptionNetworkNames[ key ].theme[ 0 ] ];
					...
					} else if ( FusionApp.settings[ fusionOptionNetworkNames[ key ].theme ] ) {
						value = FusionApp.settings[ fusionOptionNetworkNames[ key ].theme ];
				...
				// Do we have a term-option defined for this key?
				if ( fusionOptionNetworkNames[ key ].term ) {

					// If we have a post value for defined name, get its value.
					if (
						'undefined' !== typeof FusionApp.data.postMeta[ fusionOptionNetworkNames[ key ].term ] &&
						'' !== FusionApp.data.postMeta[ fusionOptionNetworkNames[ key ].term ] &&
						'default' !== FusionApp.data.postMeta[ fusionOptionNetworkNames[ key ].term ]
					) {
						value = FusionApp.data.postMeta[ fusionOptionNetworkNames[ key ].term ];
					}

					// Check for the pyre_ prefix.
					if (
						'undefined' !== typeof FusionApp.data.postMeta[ 'pyre_' + fusionOptionNetworkNames[ key ].term ] &&
						'' !== FusionApp.data.postMeta[ 'pyre_' + fusionOptionNetworkNames[ key ].term ] &&
						'default' !== FusionApp.data.postMeta[ 'pyre_' + fusionOptionNetworkNames[ key ].term ]
					) {
						value = FusionApp.data.postMeta[ 'pyre_' + fusionOptionNetworkNames[ key ].term ];
					}
				}
				...
				skip = (
					( '' === FusionApp.data.postMeta.pyre_header_bg && ( 'header_bg_repeat' === key || 'header_bg_full' === key ) ) ||
					( '' === FusionApp.data.postMeta.pyre_page_bg && ( 'page_bg_repeat' === key || 'page_bg_full' === key || 'bg_full' === key || 'bg_repeat' === key ) ) ||
					( '' === FusionApp.data.postMeta.pyre_wide_page_bg && ( 'content_bg_repeat' === key || 'content_bg_full' === key ) )
				);

				// Do we have a post-option defined for this key?
				if ( fusionOptionNetworkNames[ key ].post && ! skip ) {

					// If we have a post value for defined name, get its value.
					if (
						'undefined' !== typeof FusionApp.data.postMeta[ fusionOptionNetworkNames[ key ].post ] &&
						'' !== FusionApp.data.postMeta[ fusionOptionNetworkNames[ key ].post ] &&
						'default' !== FusionApp.data.postMeta[ fusionOptionNetworkNames[ key ].post ]
					) {
						value = FusionApp.data.postMeta[ fusionOptionNetworkNames[ key ].post ];
					}

					// Check for the pyre_ prefix.
					if (
						'undefined' !== typeof FusionApp.data.postMeta[ 'pyre_' + fusionOptionNetworkNames[ key ].post ] &&
						'' !== FusionApp.data.postMeta[ 'pyre_' + fusionOptionNetworkNames[ key ].post ] &&
						'default' !== FusionApp.data.postMeta[ 'pyre_' + fusionOptionNetworkNames[ key ].post ]
					) {
						value = FusionApp.data.postMeta[ 'pyre_' + fusionOptionNetworkNames[ key ].post ];
					}
				}
			}
		...
		generateCSS: function( id, output, cssVars, type, preview, fieldType ) {
			var values = 'TO' === type || 'FBE' === type ? FusionApp.settings : FusionApp.data.postMeta,
				self   = this,
				origValue,
				value,
				parentValue,
			...
			if ( output ) {
				...
				_.each( output, function( rule ) {
					...
					if ( 'PS' !== type ) {
						value = self.getPoToValue( id, rule.choice, type );
					}
					...
					if ( 'PO' === type && ! _.isObject( parentValue ) && ! _.isUndefined( rule.choice ) ) {
						value = values[ 'pyre_' + rule.choice ];
					}
			...
			if ( cssVars ) {
				_.each( cssVars, function( cssVar ) {
					...
					// Reset value on each loop.  In case callback of prior messed with it.
					var varVal   = self.getPoToValue( id, cssVar.choice, type ),
						selector = ':root';
		...
		updateResponsiveTypography: function( id, values ) {
			...
			if ( 'function' === typeof jQuery( '#fb-preview' )[ 0 ].contentWindow.fusionCalculateResponsiveTypeValues ) {
				jQuery( '#fb-preview' )[ 0 ].contentWindow.jQuery( 'body' ).trigger( 'fusion-typography', [ heading, values ] );
			}
		},
		...
		validateChangeContext: function( id, type ) {
			...
			_.each( fusionOptionNetworkNames, function( definition, definitionKey ) {
				...
				if ( false === found && ( definitionKey === noPyre || ( 'undefined' !== typeof definition[ networkKeys[ type ] ] && definition[ networkKeys[ type ] ] === noPyre ) ) ) {

					// Should this value override TOs?
					if ( FusionApp.data.postMeta[ key ] && '' !== FusionApp.data.postMeta[ key ] && 'default' !== FusionApp.data.postMeta[ key ] ) {
						found = [ key, type ];
					}
				}
			} );
...
( function() {
	...
	FusionPageBuilder.TabView = Backbone.View.extend( {
		...
		showHideDescription: function( event ) {
			...
			$element.closest( '.fusion-builder-option' ).find( '.description' ).slideToggle( 250 );
		...
		needsDebounce: function( event ) {
			var $target = jQuery( event.currentTarget ),
				id      = $target.parents( '.fusion-builder-option' ).data( 'option-id' ),
				fields  = this.model.get( 'fields' ),
				field   = fields[ id ];
		...
		saveChange: function( $target, value, id, save, type ) {
			...
			if ( _.isObject( save[ id ] ) && (
				...
				if ( 'variant' === $target.attr( 'name' ) ) {
					...
					// We're dealing with complex objects.
					// Get the value from the settings object.
					superVal = FusionApp.settings[ id ];
					...
					if ( _.isUndefined( superVal[ parts[ 0 ] ] ) ) {
						superVal[ parts[ 0 ] ] = [];
					}
					superVal[ parts[ 0 ] ][ parts[ 1 ] ] = value;
					save[ id ] = superVal;
				} else {
				...
				value = jQuery.parseJSON( value );
				save[ id ] = value;
			} else if ( 'pyre_sidebar_1' === id ) {
				save[ id ] = value;
				save.sbg_selected_sidebar_replacement = [ value ];
			} else if ( 'pyre_sidebar_2' === id ) {
				save[ id ] = value;
				save.sbg_selected_sidebar_2_replacement = [ value ];
			} else {
				save[ id ] = value;
			}
		...
		updatePreview: function( event, forced, alreadyTriggeredFields ) {
			var self    = this,
				...
				save    = this.getSaveLocation( type ),
			...
			if ( fields[ id ].id && 'custom_css' === fields[ id ].id ) {
				...
				return;
			} else if ( fields[ id ].id && '_fusion_builder_custom_css' === fields[ id ].id ) {
...
( function() {
	...
	_.extend( FusionPageBuilder.Callback.prototype, {
		...
		fusionEditPageSidebar: function( $trigger ) {
			var option = ( -1 < $trigger.data( 'fusion-option' ).indexOf( '_2' ) ) ? 'sidebar_2' : 'sidebar_1';
			FusionApp.sidebarView.openOption( option, 'po', $trigger.data( 'fusion-option-open-parent' ) );
		},
