...
class Fusion_Panel {
				...
				if ( ! strpos( $post_id, '-archive' ) ) {
					...
					if ( 0 !== $term_id ) {
						if ( $this->show_tax_options( $term_id ) ) {
							$option_name             = 'fusion_taxonomy_options';
							$update_taxonomy_options = update_term_meta( $term_id, $option_name, $meta_values );
	...
	public function get_archive_options() {
		...
		$option_name   = Avada_Taxonomy_Meta::get_option_name();
		$option_values = get_term_meta( get_queried_object_id(), $option_name, true );
	...
	private function get_featured_image_object( $i = '$', $post_type = '#' ) {
		...
		return [
			...
			'type'            => 'upload_id',
			'default'         => get_post_meta( fusion_library()->get_page_id(), 'kd_featured-image-' . $i . '_' . $post_type . '_id', true ),
			'location'        => 'PO',
			'not_pyre'        => true,
			'partial_refresh' => [
	...
	public function get_page_options() {
		...
		if ( is_object( $post ) ) {
			$selected_sidebar_replacement = get_post_meta( fusion_library()->get_page_id(), 'sbg_selected_sidebar_replacement', true );
			if ( ! is_array( $selected_sidebar_replacement ) ) {
				$selected_sidebar_replacement    = [];
				$selected_sidebar_replacement[0] = $selected_sidebar_replacement;
			}
			$selected_sidebar_2_replacement = get_post_meta( fusion_library()->get_page_id(), 'sbg_selected_sidebar_2_replacement', true );
			if ( ! is_array( $selected_sidebar_2_replacement ) ) {
				$selected_sidebar_2_replacement    = [];
		...
		foreach ( $ids as $id ) {
			if ( 'pyre_sidebar_1' === $id && isset( $selected_sidebar_replacement[0] ) ) {
				$values[ $id ] = $selected_sidebar_replacement[0];
			} elseif ( 'pyre_sidebar_2' === $id && isset( $selected_sidebar_2_replacement[0] ) ) {
				$values[ $id ] = $selected_sidebar_2_replacement[0];
			} elseif ( isset( $custom_fields[ $id ] ) && isset( $custom_fields[ $id ][0] ) ) {
				if ( '_fusion_google_fonts' === $id ) {
					$values[ $id ] = maybe_unserialize( $custom_fields[ $id ][0] );
				} else {
	...
	public function fusion_filter_term_meta( $value, $object_id, $meta_key, $single ) {
		$meta_values = Fusion_App()->get_data( 'meta_values' );
		if ( is_array( $meta_values ) ) {
			if ( 'fusion_taxonomy_options' === $meta_key ) {
				return [ $meta_values ];
			}
		}
		return null;
	...
	public function fusion_filter_post_meta( $value, $object_id, $meta_key, $single ) {
		...
		if ( $post_id && is_array( $meta_values ) ) {
