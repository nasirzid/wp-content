...
class Fusion_App {
	...
	public function set_data() {
		...
		$this->has_data = isset( $_POST ) && isset( $_POST['action'] ) && isset( $_POST['fusion_load_nonce'] ) && wp_verify_nonce( $_POST['fusion_load_nonce'], 'fusion_load_nonce' ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
	...
	public function preview_data() {
		global $post, $fusion_library, $current_theme_template, $template, $wp_query;
		...
		$data = [
			...
			'custom_template'      => $current_theme_template,
			...
			'is_woo_archive'       => ( ( function_exists( 'is_woocommerce' ) && is_woocommerce() && is_tax() ) || is_tax( 'product_brand' ) || is_tax( 'images_collections' ) || is_tax( 'shop_vendor' ) ),
	...
	public function set_builder_status() {
		if ( isset( $_SERVER['REQUEST_URI'] ) && false !== strpos( $_SERVER['REQUEST_URI'], 'fb-edit' ) ) { // phpcs:ignore WordPress.Security
	...
	public function live_scripts( $hook ) {
		...
		if ( ( ! defined( 'FUSION_LIBRARY_DEV_MODE' ) || ! FUSION_LIBRARY_DEV_MODE ) && ( ! defined( 'SCRIPT_DEBUG' ) || ! SCRIPT_DEBUG ) ) {
			...
			wp_enqueue_script( 'fusion-extra-functions', FUSION_LIBRARY_URL . '/inc/customizer/js/extra-functions.js', [], $fusion_library_latest_version, true );
	...
	public function fusion_app_save_post_content() {
		...
		if ( ! wp_verify_nonce( $_POST['fusion_load_nonce'], 'fusion_load_nonce' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
