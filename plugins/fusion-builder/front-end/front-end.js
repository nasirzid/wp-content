...
( function() {
	...
	FusionPageBuilder.AppView = Backbone.View.extend( {
		...
		initialize: function() {
			...
			this.inlineElements = [ 'fusion_highlight', 'fusion_tooltip', 'fusion_dropcap', 'fusion_popover', 'fusion_one_page_text_link' ];
		...
		setGlobalStyles: function( id, setAll ) {
			...
			if ( 'hundredp_padding' === id || setAllStyles ) {
				...
				styles += '.width-100 .fusion-builder-container:before,';
				styles += '.width-100 .fusion-builder-container:after,';
		...
		addPlaceholder: function( content, output ) {
			...
			// Add placeholder if no slider is selected for Revolution slider.
			if ( -1 !== content.indexOf( 'rev_slider' ) ) {
				atts = content.match( /alias\=\"(.*?)\"/ );
				notice = fusionBuilderText.slider_placeholder;
			} else if ( -1 !== content.indexOf( 'layerslider' ) ) {
				atts = content.match( /id\=\"(.*?)\"/ );
				notice = fusionBuilderText.slider_placeholder;
			}

			if ( 'undefined' !== typeof atts && ( '0' === atts[ 1 ] || '' === atts[ 1 ] ) ) {
		...
		toggleTransparentHeader: function() {
			var HeaderBGColor = '' === fusionSanitize.getPageOption( 'combined_header_bg_color' ) ? FusionApp.settings.header_bg_color : fusionSanitize.getPageOption( 'combined_header_bg_color' );
