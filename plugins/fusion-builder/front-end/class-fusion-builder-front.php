	...
	class Fusion_Builder_Front {
		...
		public function init() {
			...
			if ( fusion_is_builder_frame() ) {
				add_action( 'wp_loaded', 'fusion_builder_register_layouts' );
		...
		public function front_end_content( $content ) {
			...
			if ( is_singular( 'tribe_events' ) && ! $this->real_event_content ) {
				return $content;
			}
			...
			if ( ( $this->editable_posttype() && $this->real_event_content ) || ( ! fusion_doing_ajax() && is_main_query() && ( is_singular() || ( class_exists( 'WooCommerce' ) && is_shop() ) ) && $this->editable_posttype() ) ) {
				...
				return '<div class="fusion-builder-live-editor"></div>';
			}
		...
		public function load_templates() {
			...
			include FUSION_BUILDER_PLUGIN_DIR . '/front-end/templates/nested-column-library.php';
		...
		public function create_post() {
			...
			// Create post object.
			$my_post = [
				'post_title'   => __( 'Draft', 'fusion-builder' ) . ' ' . $pt_object->labels->singular_name,
				'post_content' => '',
				'post_status'  => 'draft',
				'post_type'    => $post_type,
			];
		...
		public function add_edit_link( $actions, $post ) {
			...
			if ( 'active' === get_post_meta( $post->ID, 'fusion_builder_status', true ) ) {
				...
				$actions['fb-front-edit'] = '<a href="' . esc_url_raw( add_query_arg( 'fb-edit', '1', get_the_permalink( $post->ID ) ) ) . '" aria-label="' . sprintf( esc_attr__( 'Edit %s with Fusion Builder Live', 'fusion-builder' ), '&#8220;' . get_the_title( $post->ID ) . '&#8221;' ) . '">' . esc_attr__( 'Fusion Builder', 'fusion-builder' ) . '</a>';
			}
			return $actions;
		}
...
function fusion_element_front_options_loop( $params ) {
	...
	<ul class="fusion-builder-module-settings {{ atts.element_type }}" data-element="{{ atts.element_type }}" data-element-cid="{{ atts.cid }}">
		...
		_.each( <?php echo $params; // phpcs:ignore WordPress.Security.EscapeOutput ?>, function(param) {
			...
			#>
			<li data-option-id="{{ param.param_name }}" data-option-type="{{ param.type }}" class="fusion-builder-option {{ param.type }}{{ hidden }}{{ childDependency }} {{optionMap}}">
						...
						<# if ( 'undefined' !== typeof param.heading ) { #>
							...
							<ul class="fusion-panel-options">
								...
								<# if ( 'undefined' !== param.default_option && '' !== param.default_option && param.default_option ) { #>
									<li><a href="JavaScript:void(0);"><span class="fusion-panel-shortcut" data-fusion-option="{{ param.default_option }}"><i class="fusiona-cog"></i></a><span class="fusion-elements-option-tooltip fusion-tooltip-global-settings"><?php echo esc_attr__( 'Theme Options', 'fusion-builder' ); ?></span></li>
								<# } #>
								<# if ( 'undefined' !== typeof param.description && 'undefined' !== typeof param.default && -1 !== param.description.indexOf( 'fusion-builder-default-reset' ) ) { #>
									<li class="fusion-builder-default-reset"> <a href="JavaScript:void(0);" class="fusion-range-default" data-default="{{ param.default }}"><i class="fusiona-undo"></i></a> <span class="fusion-elements-option-tooltip fusion-tooltip-reset-defaults"><?php echo esc_attr__( 'Reset To Default', 'fusion-builder' ); ?></span></li>
								<# } #>
								<# if ( 'undefined' !== typeof param.preview ) { #>
									...
									#>
									<li><a class="option-preview-toggle" href="JavaScript:void(0);" aria-label="<?php echo esc_attr__( 'Preview', 'fusion-builder' ); ?>" data-type="{{ dataType }}" data-selector="{{ dataSelector }}" data-toggle="{{ dataToggle }}" data-append="{{ dataAppend }}"><i class="fusiona-eye"></i></a><span class="fusion-elements-option-tooltip fusion-tooltip-preview"><?php echo esc_attr__( 'Preview', 'fusion-builder' ); ?></span></li>
								<# }; #>
							</ul>
						<# }; #>
					</div>
...
function fusion_get_image_data( $post_id, $post_featured_image_sizes = [], $post_permalink = '', $display_placeholder_image = false, $display_woo_price = false, $display_woo_buttons = false, $display_post_categories = 'default', $display_post_title = 'default', $type = '', $gallery_id = '', $display_rollover = 'yes', $display_woo_rating = false ) {
	...
	foreach ( $post_featured_image_sizes as $post_featured_image_size ) {
		...
		if ( 'related' === $type && 'fixed' === $post_featured_image_size && get_post_thumbnail_id( $post_id ) ) {
			...
			if ( has_post_thumbnail( $post_id ) ) {
				...
				$featured_image .= get_post_meta( $post_id, 'pyre_video', true );
				$featured_image .= '</div>';
	...
	return [
		...
		'image_rollover_icons'       => $image_rollover_icons,
