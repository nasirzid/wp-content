		...
		FusionPageBuilder.BaseColumnView = FusionPageBuilder.BaseView.extend( {
			...
			getTemplateAtts: function() {
				var params                 = jQuery.extend( true, {}, this.model.get( 'params' ) ),
					...
					columnSize             = '',
					style                  = '',
					classes                = 'fusion-builder-column-' + this.model.get( 'cid' ),
				...
				if ( '' !== values.background_color ) {
					alpha = 1;
					alpha = jQuery.Color( values.background_color ).alpha();
					if ( '' === values.background_image || ( 1 > alpha && 0 !== alpha ) ) {
				...
				if ( borderRadius ) {
					...
					if ( 'liftup' === values.hover_type ) {
						liftUpStyleTag = '<style type="text/css">.fusion-builder-column-' + this.model.get( 'cid' ) + ' .hover-type-liftup:before{border-radius:' + borderRadius + ';}</style>';
				...
				if ( 'yes' === values.box_shadow ) {
					values.box_shadow_horizontal = _.fusionGetValueWithUnit( values.box_shadow_horizontal );
					values.box_shadow_vertical   = _.fusionGetValueWithUnit( values.box_shadow_vertical );
					values.box_shadow_blur       = _.fusionGetValueWithUnit( values.box_shadow_blur );
					values.box_shadow_spread     = _.fusionGetValueWithUnit( values.box_shadow_spread );
					values.box_shadow            = values.box_shadow_horizontal + ' ' + values.box_shadow_vertical + ' ' + values.box_shadow_blur + ' ' + values.box_shadow_spread + ' ' + values.box_shadow_color + ' ' + values.box_shadow_style;
				...
				style += '.fusion-column-wrapper-' + this.model.get( 'cid' ) + ' {' + wrapperStyle + '}';
				style += '</style>';
