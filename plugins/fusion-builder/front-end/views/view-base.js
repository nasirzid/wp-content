	...
	jQuery( document ).ready( function() {
		...
		FusionPageBuilder.BaseView = window.wp.Backbone.View.extend( {
			...
			updateResponsiveTypography: function() {
				jQuery( '#fb-preview' )[ 0 ].contentWindow.jQuery( 'body' ).trigger( 'fusion-force-typography-update' );
			},
			...
			renderWireframePreview: function() {
				...
				if ( 'undefined' !== typeof fusionAllElements[ elementType ].preview ) {
					if ( 'undefined' === typeof this.previewView || ! this.previewView ) {
						viewSettings = {
							model: this.model,
							collection: FusionPageBuilderElements
						};
						this.previewView = new FusionPageBuilder.ElementPreviewView( viewSettings );
			...
			updateDefault: function( param, value ) {
				...
				if ( 'undefined' === typeof params[ param ] || '' === params[ param ] || 'default' === params[ param ] ) {
			...
			setInitialValue: function( param ) {
				if ( 'undefined' !== typeof this.initialValue && 'undefined' === typeof this.initialValue[ param ] && 'undefined' !== typeof param ) {
					this.initialValue[ param ] = this.model.get( 'params' )[ param ];
				}
			},
