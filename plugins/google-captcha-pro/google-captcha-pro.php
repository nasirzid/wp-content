...
if ( ! function_exists( 'gglcptch_add_admin_script_styles' ) ) {
	function gglcptch_add_admin_script_styles() {
		...
		if ( isset( $_REQUEST['page'] ) && ( 'google-captcha-pro.php' == $_REQUEST['page'] || 'google-captcha-whitelist.php' == $_REQUEST['page'] ) ) {
			...
			wp_enqueue_script( 'gglcptch_admin_script', plugins_url( 'js/admin_script.js', __FILE__ ), array(
				'jquery',
				'jquery-ui-accordion',
				'jquery-color'
			), $gglcptch_plugin_info['Version'] );
...
if ( ! function_exists( 'gglcptch_admin_footer' ) ) {
	function gglcptch_admin_footer() {
		...
		if ( isset( $_REQUEST['page'] ) && 'google-captcha-pro.php' == $_REQUEST['page'] ) {
			...
			if ( isset( $gglcptch_options['recaptcha_version'] ) && in_array( $gglcptch_options['recaptcha_version'], array(
					'v2',
					'invisible'
				) ) ) {
...
if ( ! function_exists( 'gglcptch_add_scripts' ) ) {
	function gglcptch_add_scripts() {
		...
		wp_enqueue_script( 'gglcptch_script', plugins_url( 'js/script.js', __FILE__ ), array(
			'jquery',
			'gglcptch_api'
		), $gglcptch_plugin_info['Version'], true );
...
if ( ! function_exists( 'gglcptch_get_language' ) ) {
	function gglcptch_get_language() {
		...
		if (
			...
			} elseif ( array_key_exists( $gglcptch_multilanguage_language_short, $gglcptch_languages ) ) {
	...
	function gglcptch_test_keys() {
		...
		if ( isset( $_REQUEST['_wpnonce'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], $_REQUEST['action'] ) ) {
			...
			*/
			...
			<p>
				<input type="hidden" name="gglcptch_is_network"
						value="<?php echo ! empty( $_REQUEST['is_network'] ) ? '1' : '0'; ?>"/>
				<input type="hidden" name="gglcptch_test_keys_verification-nonce"
						value="<?php echo wp_create_nonce( 'gglcptch_test_keys_verification' ); ?>"/>
				<button id="gglcptch_test_keys_verification" name="action" class="button-primary"
						value="gglcptch_test_keys_verification"><?php _e( 'Test verification', 'google-captcha-pro' ); ?></button>
			</p>
		<?php }
		die();
	}
		...
		if ( isset( $_REQUEST['_wpnonce'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], $_REQUEST['action'] ) ) {
			...
			*/
			...
			if ( ! $result['response'] ) {
				...
				<div class="updated gglcptch-test-results below-h2">
					<p><?php _e( 'The verification is successfully completed.', 'google-captcha-pro' ); ?></p></div>
