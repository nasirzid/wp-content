...
class WC_PIP_Orders_Admin {
	...
	public function get_print_confirmation_message( $document, $order_ids, $redirect_url = '' ) {
		...
		$link_class = $orders_count > 1 ? 'wc-pip-print wc-pip-print-many' : 'wc-pip-print';
		$print_link = '<a href="' . $action_url .'" target="_blank" class="' . $link_class . '">' . __( 'Print now.', 'woocommerce-pip' ) . '</a>';
	...
	public function display_order_invoice_number( $wc_order ) {
		...
		$order_id = $wc_order instanceof \WC_Order ? Framework\SV_WC_Order_Compatibility::get_prop( $wc_order, 'id' ) : null;
	...
	public function add_order_preview_invoice_number( $data, $order ) {
		...
		if ( $order ) {
			...
			$invoice_number = get_post_meta( Framework\SV_WC_Order_Compatibility::get_prop( $order, 'id' ), '_pip_invoice_number', true );
	...
	public function add_order_actions( $actions, $order ) {
		...
		if ( $wc_order instanceof \WC_Order && wc_pip()->get_handler_instance()->current_admin_user_can_manage_documents() ) {
			...
			$order_id = Framework\SV_WC_Order_Compatibility::get_prop( $wc_order, 'id' );
