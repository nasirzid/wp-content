...
class WC_PIP_Integration_VAT_Number {
	...
	/**
	 * Get the VAT Number set on the order provided by checking if any known meta key exists.
	 *
	 * @since 3.1.5
	...
	private function get_vat_number( $order_id ) {
		...
		if ( $order instanceof \WC_Order ) {
			 ...
			 */
			$vat_number_meta_keys = (array) apply_filters( 'wc_pip_vat_number_meta_keys', array(
			...
			foreach ( $vat_number_meta_keys as $meta_key ) {
				...
				$vat_number = Framework\SV_WC_Order_Compatibility::get_meta( $order, $meta_key, true );
