...
abstract class WC_PIP_Document {
	...
	public function __construct( $args ) {
		...
		if ( $this->order instanceof \WC_Order ) {
			...
			$this->order_id    = Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'id' );
	...
	public function output_template( $args = array() ) {
		...
		if ( ! empty( $this->order_ids ) && is_array( $this->order_ids ) ) {
			...
			foreach ( $this->order_ids as $order_id ) {
				...
				$template_args['order_id'] = $this->order_id = Framework\SV_WC_Order_Compatibility::get_prop( $wc_order, 'id' );
			...
			$template_args['order_id'] = $this->order_id = Framework\SV_WC_Order_Compatibility::get_prop( $original_order, 'id' );
	...
	public function get_customer_note() {
		...
		$customer_note = $this->order instanceof \WC_Order ? Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'customer_note' ) : null;
	...
	public function get_invoice_date() {
		...
		$order_datetime = Framework\SV_WC_Order_Compatibility::get_date_created( $this->order );
	...
	public function has_invoice_number() {
		...
		if ( $this->order_id > 0 && ( $order = wc_get_order( $this->order_id ) ) ) {
			$invoice_number     = Framework\SV_WC_Order_Compatibility::get_meta( $order, '_pip_invoice_number', true );
	...
	public function get_table_rows() {
		...
		if ( $this->get_items_count() > 0 || ( 'invoice' === $this->type && $this->get_items_count() >= 0 ) ) {
			 ...
			 * @param string $type Document type
			 */
			$sort_alphabetically = apply_filters( 'wc_pip_document_sort_order_items_alphabetically', true, Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'id' ), $this->type );
		 ...
		 */
		return apply_filters( 'wc_pip_document_table_rows', $table_rows, $items, Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'id' ), $this->type, $this );
	}
	...
	protected function get_table_row_order_item_data( $item_id, $item ) {
		...
		$product   = Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ? $item->get_product() : $this->order->get_product_from_item( $item );
		...
		if ( ! $this->order instanceof \WC_Order ) {
			...
			if ( $this->show_prices_excluding_tax ) {
				...
				if ( Framework\SV_WC_Plugin_Compatibility::is_wc_version_lt_3_0() ) {
					$order_tax_display_cart_value  = $this->order->tax_display_cart;
					$this->order->tax_display_cart = 'excl';
				}
			...
			if ( $this->show_prices_excluding_tax ) {
				...
				if ( isset( $order_tax_display_cart_value ) && Framework\SV_WC_Plugin_Compatibility::is_wc_version_lt_3_0() ) {
					$this->order->tax_display_cart = $order_tax_display_cart_value;
				}
	...
	/**
	 * Returns an item price refunded amount in HTML.
	 *
	...
	private function get_order_item_refunded_price_html( $item_id, $item ) {
		...
		if ( $refund_amount > 0 ) {
			...
			$refund_total = wc_price( max( 0, $item_total - $refund_amount ), array(
				'currency'     => Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ? $this->order->get_currency() : $this->order->get_order_currency(),
	...
	protected function get_order_item_weight_html( $item_id, $item, $product ) {
		...
		$product_weight = $product ? Framework\SV_WC_Product_Compatibility::get_prop( $product, 'weight', 'view' ) : 0;
	...
	private function get_document_action_count( $action ) {
		...
		// Get count.
		return max( 0, (int) Framework\SV_WC_Order_Compatibility::get_meta( $this->order, "_wc_pip_{$document_type}_{$action}_count", true ) );
	}
		...
		if ( $this->order_ids && is_array( $this->order_ids ) ) {
			...
			foreach ( $this->order_ids as $order_id ) {
				...
				if ( $order = wc_get_order( $order_id ) ) {
					...
					Framework\SV_WC_Order_Compatibility::update_meta_data( $order, "_wc_pip_{$document_type}_{$action}_count", $amount );
			...
			Framework\SV_WC_Order_Compatibility::update_meta_data( $this->order, "_wc_pip_{$document_type}_{$action}_count", $amount );
