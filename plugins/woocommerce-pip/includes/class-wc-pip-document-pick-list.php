...
class WC_PIP_Document_Pick_List extends WC_PIP_Document_Packing_List {
	...
	public function __construct( array $args ) {
		...
		add_filter( 'wc_pip_sort_order_item_rows', array( $this, 'sort_order_item_rows' ), 10, 3 );
	}
	...
	protected function add_table_order_heading() {
		...
		$edit_post_url   = get_edit_post_link( Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'id' ) );
			...
			if ( ! empty( $item_data['orders'] ) ) {
				...
				foreach ( $item_data['orders'] as $order ) {
					...
					if ( $has_product && true === $show_purchase_note && $order->is_paid() ) {
						...
						$purchase_note = Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ? get_post_meta( $product->get_id(), '_purchase_note', true ) : $product->get_purchase_note();
	...
	protected function get_category_items_total_weight_html( $item_data ) {
		...
		$product        = $item_data['product'];
		$total_weight   = 0;
		$product_weight = Framework\SV_WC_Product_Compatibility::get_prop( $product, 'weight', 'view' );
		...
		foreach ( $this->order_ids as $order_id ) {
			...
			if ( $this->order && $this->get_items_count() > 0 ) {
				...
				foreach ( $items as $item_id => $item ) {
					...
					$product_id = $product->is_type( 'variation' ) ? Framework\SV_WC_Product_Compatibility::get_parent( $product )->get_id() : $product->get_id();
					 ...
					 */
					$group_as_uncategorized = (bool) apply_filters( 'wc_pip_pick_list_group_item_as_uncategorized', false, $item, $this->order );
					...
					$cat_table_rows[ $key ][ $meta_key ]['product']                  = Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ? $item->get_product() : $this->order->get_product_from_item( $item );
				}
			}
		}

		/** This filter is documented in includes/abstract-wc-pip-document.php */
		$sort_alphabetically = (bool) apply_filters( 'wc_pip_document_sort_order_items_alphabetically', true, Framework\SV_WC_Order_Compatibility::get_prop( $this->order, 'id' ), $this->type );
