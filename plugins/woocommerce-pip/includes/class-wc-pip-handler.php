...
class WC_PIP_Handler {
	...
	public function __construct() {
		...
		$statuses = $this->get_order_paid_statuses();
	...
	/**
	 * Get order paid statuses.
	 *
	 * @since 3.2.0
	 * @return array
	 */
	private function get_order_paid_statuses() {
		...
		if ( $document || false !== $this->generate_invoice_number_on_order_paid() ) {
			...
			if ( $document instanceof \WC_PIP_Document && $order instanceof \WC_Order ) {
				...
				if ( ! $document->has_invoice_number() ) {
					...
					Framework\SV_WC_Order_Compatibility::add_meta_data( $order, '_pip_invoice_number', $invoice_number, true );
