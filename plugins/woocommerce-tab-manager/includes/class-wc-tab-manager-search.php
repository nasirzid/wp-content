...
class WC_Tab_Manager_Search extends WP_Query {
	...
	/**
	 * Modifies the main query on product pages when the tab layout filter is
	 * in use.
	 *
	 * @since 1.4.0
	...
	public function update_products_for_tabs( $tab_id_list, $args = array() ) {
		...
		foreach ( $product_id_list as $product_id ) {
			...
			if ( $product = wc_get_product( $product_id ) ) {
				$override = SV_WC_Product_Compatibility::get_meta( $product, '_override_tab_layout', true );
			}
			...
			if ( ! empty( $combined_content ) && 'update' === $action ) {
				...
				foreach ( $combined_content as $tab_type => $content ) {
					...
					if ( $product = wc_get_product( $product_id ) ) {
						...
						// TODO: consider switching back to SV_WC_Product_Compatibility::update_meta_data( $product, $meta_key, $content ); when
						// WC has fixed https://github.com/woocommerce/woocommerce/issues/15274 and/or https://github.com/woocommerce/woocommerce/pull/14751 {IT 2017-05-29}
						update_post_meta( $product_id, $meta_key, $content );
					}
				}
			}
